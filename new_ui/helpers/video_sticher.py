...
def stitch_video_from_segments(segment_list, course: str, srt_filename="./data/output/stitched_output.srt", pause_duration=0.01):
    clips = []
    clips_info = []
    sources = set()
    ...
    for idx, segment in enumerate(segment_list):
        ...
        if not filename or not timestamp or not sentence:
            print(f"Skipping invalid segment: {segment}")
            continue

        lecture_no = os.path.splitext(filename)[0]
        video_file = f"./data/videos/{course}/{lecture_no}.mp4"
        start, end = parse_timestamp(timestamp)
        sources.add(f"Lecture - {lecture_no}")

        try:
            clip = VideoFileClip(video_file).subclip(start, end)

            # Add a short black pause between clips
            if idx > 0:
